...
class RadiantMob:
    ...
    def create_new_path_square(self, dx: int, dy: int):
        sim_x, sim_y = self.x, self.y

        full_path = [(sim_x, sim_y)]

        while sim_x != dx or sim_y != dy:

            full_path.append((sim_x, sim_y))
    ...
    def update_path_square(self):
        self.path = self.create_new_path_square(self.desired_x, self.desired_y)
